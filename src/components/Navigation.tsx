...
import { Users, Calendar, BarChart3, Settings, Menu, X, Moon, Sun, Bell, LogOut } from 'lucide-react';
...
const Navigation: React.FC<NavigationProps> = ({
  ...
  const { darkMode, setDarkMode, user, logout } = useAppStore();
            ...
            <div className="bg-gradient-to-r from-blue-50 to-teal-50 dark:from-blue-900/20 dark:to-teal-900/20 p-4 rounded-lg border border-blue-100 dark:border-blue-800">
              <div className="flex items-center space-x-3 mb-2">
                <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-teal-500 rounded-full flex items-center justify-center">
                  <span className="text-white font-semibold text-sm">{user.name ? user.name.split(' ').map(n => n[0]).join('') : 'U'}</span>
                </div>
                <div>
